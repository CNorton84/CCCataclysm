...
int iuse::anticonvulsant( player *p, item *it, bool, const tripoint & )
{
    ...
    if (p->has_effect(effect_seizure)) {
        p->remove_effect(effect_seizure);
        p->add_msg_if_player(m_good, _("The tingling stops.  No seizures, for now."));
    }
    return it->type->charges_to_use();
}
...
int iuse::oxytorch( player *p, item *it, bool, const tripoint & )
{
    ...
    } else if(furn == f_safe_c || f_safe_l) {
